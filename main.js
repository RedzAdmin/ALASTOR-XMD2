...
const blocklistCommand = require('./commands/blocklist');
const antispamCommand = require('./commands/antispam');
const vcfCommand = require('./commands/vcf');
const kickinactiveCommand = require('./commands/kickinactive');
const listactiveCommand = require('./commands/listactive');
const listinactiveCommand = require('./commands/listinactive');
const saveCommand = require('./commands/save');
const extractaudioCommand = require('./commands/extractaudio');

// Global settings
global.packname = settings.packname;
global.author = settings.author;
global.channelLink = "https://whatsapp.com/channel/0029Vb70IdY60eBmvtGRT00R";
global.ytch = "Hacking Unveiled";
    ...
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                ...
                await sock.sendMessage(chatId, { 
                    text: `🔗 *Support*\n\nhttps://chat.whatsapp.com/EgWYHIPowuS1wv54JII6Ff` 
                }, { quoted: message });
                return;
            }
        }
        ...
        // Anti-spam check
        if (await antispamCommand.checkSpam(sock, message)) {
            return; // Block spam messages
        }
        ...
        const adminCommands = ['.mute', '.unmute', '.ban', '.unban', '.promote', '.demote', '.kick', '.tagall', '.tagnotadmin', '.hidetag', '.antilink', '.antitag', '.setgdesc', '.setgname', '.setgpp', '.kickinactive'];
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.kickinactive'):
                if (!isGroup) {
            ...
            case userMessage.startsWith('.listactive'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '❌ This command only works in groups!' });
                    break;
                }
            ...
            case userMessage.startsWith('.listinactive'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '❌ This command only works in groups!' });
                    break;
                }
            ...
            case userMessage.startsWith('.mp3'):
                await extractaudioCommand.start(sock, m, { 
                    ...
                    prefix: '.',
                    quoted: m.quoted,
                    mime: message.message?.videoMessage ? 'video' : message.message?.audioMessage ? 'audio' : null
                });
                commandExecuted = true;
                break;
            ...
            case userMessage.startsWith('.unban'):
                ...
                break;
            case userMessage === '.help' || userMessage === '.menu' || userMessage === '.bot' || userMessage === '.alastor':
